    ...
    while (1) {
        ...
        if (c < 0) {
            switch (get_e()) {
...
#endif
                case EINPROGRESS:
                    // Очередь accept опустела — все входящие приняты, это норма.
                    return 0;
                ...
                case ENOBUFS:
                case ENOMEM:
                    // ТРАНЗИЕНТНАЯ ошибка на ОДНОМ входящем соединении. Частый
                    // случай — клиент оборвал сокет между SYN и accept, когда на
                    // холодный, только что поднятый прокси прилетает ПАЧКА
                    // параллельных подключений (наши пробы: yandex+youtube+
                    // googlevideo TLS + SOCKS-контроль QUIC — 4 коннекта разом).
                    // РАНЬШЕ здесь возвращался -1, а вызывающий код в event_loop
                    // по этому -1 ставил NOT_EXIT=0 и глушил ВЕСЬ прокси на
                    // середине кандидата. Из-за этого часть параллельных проб
                    // (успевшие хуже всех — обычно googlevideo) ложно получала
                    // TLS_TIMEOUT, а рабочий флаг помечался FAILED.
                    // epoll тут level-triggered (без EPOLLET) → оставшийся бэклог
                    // заново поднимет EV_ACCEPT. Просто выходим — сервер жив.
                    uniperror("accept (transient, ignored)");
                    return 0;

                default:
                    // Настоящий крах слушателя (EBADF/EINVAL/ENOTSOCK) —
                    // останавливаем цикл. Штатный stopProxy глушит цикл через
                    // POLLHUP от shutdown(), сюда попадают только аномалии.
                    uniperror("accept");
                    return -1;
            }
        }
